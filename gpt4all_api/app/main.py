...
async def startup():
    ...
    if settings.inference_mode == "cpu":
        ...
        model = GPT4All(model_name=settings.model, model_path=settings.gpt4all_path, allow_download=False, device="cpu")
...
@app.post("/gpt4all/prompt")
async def root(prompt_req: PromptRequest):
    system_template = 'A chat between a curious user and an artificial intelligence assistant.'
    # many models use triple hash '###' for keywords, Vicunas are simpler:
    prompt_template = 'USER: {0}\nASSISTANT: '
    with model.chat_session(system_template, prompt_template):
        response1 = model.generate(prompt_req.prompt)
    return {"message": response1}
...
def init_eureka_client():
    your_rest_server_port = 4891
    eureka_client.init(eureka_server="http://insane-service-registry:8761/eureka/",
                                    app_name="insane-gpt4all-app",
                                    instance_port=your_rest_server_port)

eureka_thread = threading.Thread(target=init_eureka_client)
eureka_thread.start()
